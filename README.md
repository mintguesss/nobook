...
把上課用的 PDF / PPTX 放進 `materials/`，**照課程分資料夾**：
...
**一定要分資料夾。** 攤平放的話每堂課都會拿去比對所有教材——實測管理資訊
系統那堂跑去對照生產與作業管理的投影片，而且分數還不低（同樣是管理學院的
課，用詞重疊）。放在根目錄的檔案會被列成「未分類」，不會拿來對照。

在歷史紀錄點進某一堂 → 投影片對照 → 對照投影片。純 CPU，一堂課一兩秒。
筆記的段落標題會標上頁碼，投影片對照裡的頁碼可以點回對應的筆記。

涵蓋率實測 **18–24%**：有對到的部分準確且單調遞增，對不上的是老師講投影片
以外的內容、Q&A，或那幾頁只有圖。不要期待整堂課的頁碼地圖。
